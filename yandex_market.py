...
class YMClient:
    ...
    def __init__(self, client_id, token, campaign_id):
        ...
        self.vat_values = {2: 0.1, 5: 0, 6: None, 7: 0.2}
    ...
    def get_prices(self):
        ...
        for offer in offers:
            ...
            old_price = offer['price']['discountBase']
            vat = offer['price'].get('vat')
            if vat:
                ...
                for offer in price_suggestions:
                    ...
                    for suggestion in suggestions:
                        if suggestion.get('type') == 'BUYBOX':
                            df.loc[df['product_id'] == product_id, 'recommended_price'] = suggestion['price']
            counter += 1
        # Add tariffs to df.
                ...
                for tariff in tariffs:
                    if tariff.get('type') == 'FEE':
                        df.loc[(df['product_id'] == product_id) & (df['offer_id'] == offer_id),
                               'sales_percent'] = tariff['percent']
                    if tariff.get('type') == 'FULFILLMENT':
                        df.loc[(df['product_id'] == product_id) & (df['offer_id'] == offer_id),
                               'fbo_fulfillment_amount'] = tariff['amount']
                    if tariff.get('type') == 'AGENCY_COMMISSION':
                        df.loc[(df['product_id'] == product_id) & (df['offer_id'] == offer_id),
                               'equering_cost'] = tariff['percent']
